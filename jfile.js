...
function continuous(evt)
{
        ...
        cancx.beginPath(evt.clientX,evt.clientY); // corrects the path 
        cancx.lineWidth=width;
        cancx.strokeStyle=color;
		
		test("tests that the shape's coordinates are not the coordinates of the begining of the freedraw.", function(){	
			notEqual(pathx, 100);// the x and y coordinates are hard coded to 100, 100
			notEqual(pathy, 100);
			})
		...
		//The offset of pathx and pathy originally make a line draw vertically on the canvas 
}
